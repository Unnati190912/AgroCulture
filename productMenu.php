				...
				<?php
					...
					<?php
						...
						while($row = $result->fetch_array()):
							// Map product names to image files in ImagesAg directory
							$imageMap = [
								'Mango' => 'mango1.JPG',
							...
							// Get the appropriate image name based on product name
							$imageName = 'default.jpg';
							foreach ($imageMap as $productName => $imgFile) {
								if (stripos($row['product'], $productName) !== false) {
									$imageName = $imgFile;
									break;
								}
							}
							
							$picDestination = "ImagesAg/".$imageName;
							// Check if image exists, otherwise use a default image
							if (!file_exists($picDestination)) {
								$picDestination = "ImagesAg/default.jpg";
							}
						?>
							<div class="col-md-4">
							<section>
							...
							<a href="review.php?pid=<?php echo $row['pid'] ;?>" > 
								<img class="image fit" src="<?php echo $picDestination;?>" height="220px;" alt="<?php echo $row['product']; ?>" onerror="this.src='ImagesAg/default.jpg';" />
							</a>
